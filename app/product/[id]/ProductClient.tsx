...
function ProductDetailContent({ id, country, initialProduct }: Props) {
    ...
    const handlePlus = () => {
        if (pageQuantity >= effectiveMaxQty) {
            toast(RU_DICTIONARY.productPage.maximumStockReached, { icon: '⚠️' });
            return;
        }
                    ...
                    <div className="space-y-1.5 pt-0 pr-0 lg:pr-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {product.is_best_seller && (
                                <span className="inline-flex items-center justify-center bg-[#91C934] text-white text-[10px] font-extrabold px-3 py-1 rounded-md uppercase tracking-wider leading-none">{RU_DICTIONARY.productPage.bestseller}</span>
                            )}
                            {product.is_new_arrival && (
                                <span className="inline-flex items-center justify-center bg-[#FF0000] text-white text-[10px] font-extrabold px-3 py-1 rounded-md uppercase tracking-wider leading-none">{RU_DICTIONARY.productPage.newArrival}</span>
                            )}
                        </div>
                        ...
                        {(Number(ratingSummary?.average_rating || product.avg_rating || 0) > 0 || (Number(ratingSummary?.total_reviews || product.review_count || 0) > 0)) ? (
                            <div className="flex items-center gap-2 flex-wrap">
                                ...
                                {(ratingSummary?.total_reviews || Number(product.review_count || 0)) > 0 && (
                                    <span className="text-sm text-gray-500">({Number(ratingSummary?.total_reviews || product.review_count || 0).toLocaleString()} {(ratingSummary?.total_reviews || Number(product.review_count || 0)) !== 1 ? RU_DICTIONARY.productPage.reviews : RU_DICTIONARY.productPage.review})</span>
                                )}
                                {Number((product as any).total_sold || 0) > 0 && (
                                    <span className="text-sm text-gray-500 flex items-center gap-1">
                                        <span className="text-green-600">●</span> {(product as any).total_sold}+ {RU_DICTIONARY.productPage.boughtThisMonth}
                                    </span>
                                )}
                            </div>
                        ) : (
                            <p className="text-sm text-gray-400 italic">{RU_DICTIONARY.productPage.noReviewsYet}</p>
                        )}
                        ...
                        <div className="p-4 rounded-xl bg-white shadow-[inset_0_0_0_1px_#E9ECEF] space-y-1">
                                ...
                                {isDiscounted && (
                                    <>
                                        <div className="text-[#1e3a8a] text-[13px] leading-none mb-1">
                                            <span className="font-black">{RU_DICTIONARY.productPage.wow}</span> <span className="font-semibold">{RU_DICTIONARY.productPage.limitedTimeOffer}</span>
                                        </div>
                                        <div className="flex items-center gap-3">
                                            <span className="bg-[#C1262D] text-white text-xs font-bold px-2 py-0.5 rounded-sm">
                                                {discountPercent}% {RU_DICTIONARY.product.off}
                                            </span>
                            ...
                            {selectedVariant?.size_label && selectedVariant.size_label.toLowerCase() !== 'standard' && (
                                <p className="text-sm text-gray-500">{RU_DICTIONARY.productPage.inclusiveOfTaxes}</p>
                            )}
                        ...
                        <div className="flex items-center justify-between border border-dashed border-gray-300 bg-[#FFFFFF] rounded-xl p-1 transition-all">
                            <div className="flex items-center gap-3">
                                ...
                                <div className="flex flex-col sm:flex-row sm:items-center sm:gap-2">
                                    <span className="text-[15px] font-bold text-gray-800">{RU_DICTIONARY.productPage.extra10Off}</span>
                                    <span className="text-sm text-gray-500">{RU_DICTIONARY.productPage.onlyForNewUsers}</span>
                                </div>
                            </div>
                            <div className="bg-white px-3 py-1 rounded-lg border border-gray-200 shadow-sm">
                                <span className="text-[13px] text-[#91C934] font-bold tracking-wider">{RU_DICTIONARY.productPage.codeNewUser}</span>
                            </div>
                        </div>
                            ...
                            <div className="flex items-start gap-3">
                                ...
                                <div className="flex-1">
                                    <p className="text-sm text-gray-800">
                                        {RU_DICTIONARY.productPage.guaranteedDelivery} <span className="font-bold text-[#91C934]">14 {RU_DICTIONARY.productPage.daysOfOrdering}</span>
                                    </p>
                        ...
                        <div className="min-h-[20px]">
                            {variants.length > 0 && (() => {
                                ...
                                const formatVolume = (ml: number) => {
                                    return ml >= 999 ? `${(ml / 1000).toFixed(ml % 1000 === 0 ? 0 : 1)} л` : `${ml} мл`;
                                };

                                const formatWeight = (g: number) => {
                                    return g >= 1000 ? `${(g / 1000).toFixed(g % 1000 === 0 ? 0 : g % 100 === 0 ? 1 : 2)} кг` : `${Math.round(g)} г`;
                                };
                                ...
                                return (
                                    <div className="space-y-4">
                                            ...
                                            <div className="flex flex-wrap items-center gap-x-6 gap-y-2 mb-6 pb-4 border-b border-gray-100">
                                                {hasSingleWeight && (
                                                    <div className="flex items-baseline">
                                                        <span className="text-sm text-gray-900 mr-2 font-medium">{RU_DICTIONARY.productPage.weight}</span>
                                                        <span className="text-sm font-medium text-gray-900">{formatWeight(uniqueWeights[0])}</span>
                                                    </div>
                                                )}
                                                {hasSingleStrength && (
                                                    <div className="flex items-baseline">
                                                        <span className="text-sm text-gray-900 mr-2 font-medium">{RU_DICTIONARY.productPage.strength}</span>
                                                        <span className="text-sm font-medium text-gray-900">{uniqueStrengths[0]}</span>
                                                    </div>
                                                )}
                                                {hasSingleVolume && (
                                                    <div className="flex items-baseline">
                                                        <span className="text-sm text-gray-900 mr-2 font-medium">{RU_DICTIONARY.productPage.volume}</span>
                                                        <span className="text-sm font-medium text-gray-900">{formatVolume(uniqueVolumes[0])}</span>
                                                    </div>
                                                )}
                                                {hasSingleCount && (
                                                    <div className="flex items-baseline">
                                                        <span className="text-sm text-gray-900 mr-2 font-medium">{RU_DICTIONARY.productPage.count}</span>
                                                        <span className="text-sm font-medium text-gray-900">{uniqueCounts[0]}</span>
                                                    </div>
                                                )}
                                                {hasSingleFlavor && (
                                                    <div className="flex items-baseline">
                                                        <span className="text-sm text-gray-900 mr-2 font-medium">{RU_DICTIONARY.productPage.flavour}</span>
                                                        <span className="text-sm font-medium text-gray-900 capitalize">{uniqueFlavors[0]}</span>
                                                    </div>
                                                )}
                                                {hasSinglePack && (
                                                    <div className="flex items-baseline">
                                                        <span className="text-sm text-gray-900 mr-2 font-medium">{RU_DICTIONARY.productPage.packOf}</span>
                                                        <span className="text-sm font-medium text-gray-900">{RU_DICTIONARY.productPage.packOf} {uniquePacks[0]}</span>
                                                    </div>
                                                )}
                                            </div>
                                        )}
                                        ...
                                        {uniqueWeights.length > 1 && (
                                            <div className="flex items-start gap-3">
                                                <p className="text-sm font-bold text-gray-900 w-[65px] pt-2.5 flex-shrink-0">{RU_DICTIONARY.productPage.size}</p>
                                        ...
                                        {uniqueStrengths.length > 1 && (
                                            <div className="flex items-start gap-3">
                                                <p className="text-sm font-bold text-gray-900 w-[65px] pt-2.5 flex-shrink-0">{RU_DICTIONARY.productPage.strength}</p>
                                                <div className="flex gap-2.5 flex-wrap flex-1">
                                        ...
                                        {uniqueVolumes.length > 1 && (
                                            <div className="flex items-start gap-3">
                                                <p className="text-sm font-bold text-gray-900 w-[65px] pt-2.5 flex-shrink-0">{RU_DICTIONARY.productPage.volume}</p>
                                                <div className="flex gap-2.5 flex-wrap flex-1">
                                        ...
                                        {uniqueCounts.length > 1 && (
                                            <div className="flex items-start gap-3">
                                                <p className="text-sm font-bold text-gray-900 w-[65px] pt-2.5 flex-shrink-0">{RU_DICTIONARY.productPage.count}</p>
                                                <div className="flex gap-2.5 flex-wrap flex-1">
                                        ...
                                        {uniqueFlavors.length > 1 && (
                                            <div className="flex items-start gap-3">
                                                <p className="text-sm font-bold text-gray-900 w-[65px] pt-2.5 flex-shrink-0">{RU_DICTIONARY.productPage.flavour}</p>
                                                <div className="flex gap-2.5 flex-wrap flex-1">
                                        ...
                                        {uniquePacks.length > 1 && (
                                            <div className="flex items-start gap-3">
                                                <p className="text-sm font-bold text-gray-900 w-[65px] pt-3 flex-shrink-0">{RU_DICTIONARY.productPage.packOf}</p>
                                                <div className="flex gap-2.5 flex-wrap flex-1">
                            ...
                            <div className="flex items-center gap-4 py-2">
                                {isComingSoon ? (
                                    <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold bg-purple-50 text-purple-700 border border-purple-200">
                                        🕒 {RU_DICTIONARY.productPage.comingSoon}
                                    </div>
                                ) : isExpired ? (
                                    <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold bg-red-50 text-red-600 border border-red-200">
                                        <AlertTriangle size={12} />{RU_DICTIONARY.productPage.availabilityEnded}</div>
                                ) : isOutOfStock && (
                                    <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold bg-red-50 text-red-600 border border-red-200">
                                        <AlertTriangle size={12} />{RU_DICTIONARY.product.outOfStock}</div>
                                )}
                            </div>
                        )}
                            ...
                            {isOutOfStock ? (
                                <div className="bg-red-50 p-4 rounded-xl border border-red-100 flex flex-col gap-3">
                                    <div className="flex items-center gap-2 text-red-700 font-semibold mb-1">
                                        ...
                                        <span>{RU_DICTIONARY.productPage.notifyMeWhenBackInStock}</span>
                                    </div>
                                    <div className="flex gap-2">
                                        <input
                                            type="email"
                                            placeholder={RU_DICTIONARY.productPage.enterEmailAddress}
                                        ...
                                        >
                                            {isRestockNotifying ? <span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin inline-block" /> : RU_DICTIONARY.productPage.notifyMe}
                                        </button>
                                    </div>
                                </div>
                            ) : existingCartItem ? (
                                <div className="grid grid-cols-[auto_1fr_1fr] gap-3">
                                        ...
                                        <button
                                            onClick={async () => {
                                                ...
                                                if (newQty <= 0) {
                                                    ...
                                                    toast.success(RU_DICTIONARY.productPage.removedFromCart);
                                                } else {
                                    ...
                                    <button
                                        onClick={() => router.push(buildPath(country, '/cart'))}
                                    ...
                                    >
                                        <ShoppingCart size={18} className="text-[#91C934]" />{RU_DICTIONARY.productPage.viewCart}</button>
                                    <button
                            ...
                            <div className="bg-[#f0f4ef] rounded-2xl p-4 mb-8">
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                                    <div className="flex flex-col items-center text-center gap-2">
                                        ...
                                        <span className="text-[11px] font-bold uppercase tracking-wider text-gray-700 leading-tight">{RU_DICTIONARY.productPage.dermatologicallyTested}</span>
                                    </div>
                                    <div className="flex flex-col items-center text-center gap-2">
                                        <div className="w-12 h-12 rounded-full bg-white flex items-center justify-center text-[#91C934] shadow-sm">
                                            <CheckCircle2 size={24} />
                                        </div>
                                        <span className="text-[11px] font-bold uppercase tracking-wider text-gray-700 leading-tight">{RU_DICTIONARY.productPage.safeForAllTypes}</span>
                                    </div>
                                    <div className="flex flex-col items-center text-center gap-2">
                                        <div className="w-12 h-12 rounded-full bg-white flex items-center justify-center text-[#91C934] shadow-sm">
                                            <Award size={24} />
                                        </div>
                                        <span className="text-[11px] font-bold uppercase tracking-wider text-gray-700 leading-tight">{RU_DICTIONARY.productPage.crueltyFree}</span>
                                    </div>
                                    <div className="flex flex-col items-center text-center gap-2">
                                        <div className="w-12 h-12 rounded-full bg-white flex items-center justify-center text-[#91C934] shadow-sm">
                                            <LeafIcon size={24} />
                                        </div>
                                        <span className="text-[11px] font-bold uppercase tracking-wider text-gray-700 leading-tight">{RU_DICTIONARY.productPage.natural100}</span>
                                    </div>
                                </div>
                            </div>

                            <h2 className="text-2xl font-bold text-gray-900 mb-6">{RU_DICTIONARY.productPage.productDescription}</h2>
                                ...
                                {activeInfoTab === 'description' && product.description && (
                                    <div className="animate-fadeIn">
                                        ...
                                        {product.description.split(/\s+/).length > 150 && (
                                            ...
                                            >
                                                {isDescriptionExpanded ? RU_DICTIONARY.productPage.readLess : RU_DICTIONARY.productPage.readMore}
                                            </button>
                                        )}
                                        ...
                                        {product.specialities && product.specialities.length > 0 && (
                                            <div className="mt-8 pt-6 border-t border-gray-100">
                                                <p className="text-sm font-bold text-gray-900 mb-3">{RU_DICTIONARY.productPage.features}</p>
                                        ...
                                        <div className="divide-y divide-gray-100">
                                            {product.brand && (
                                                <div className="flex py-3">
                                                    <span className="text-sm text-gray-500 w-40 flex-shrink-0 font-medium">{RU_DICTIONARY.productPage.brand}</span>
                                                    <span className="text-sm text-gray-900 font-semibold">{product.brand}</span>
                                                </div>
                                            )}
                                            {product.category && (
                                                <div className="flex py-3">
                                                    <span className="text-sm text-gray-500 w-40 flex-shrink-0 font-medium">{RU_DICTIONARY.productPage.category}</span>
                                                    <span className="text-sm text-gray-900">{product.category}</span>
                                                </div>
                                            )}
                                            {product.form && (
                                                <div className="flex py-3">
                                                    <span className="text-sm text-gray-500 w-40 flex-shrink-0 font-medium">{RU_DICTIONARY.productPage.form}</span>
                                                    <span className="text-sm text-gray-900 capitalize">{product.form}</span>
                                                </div>
                                            )}
                                            {product.country_of_origin && (
                                                <div className="flex py-3">
                                                    <span className="text-sm text-gray-500 w-40 flex-shrink-0 font-medium">{RU_DICTIONARY.productPage.countryOfOrigin}</span>
                                            ...
                                            {product.unit_of_measure && (
                                                <div className="flex py-3">
                                                    <span className="text-sm text-gray-500 w-40 flex-shrink-0 font-medium">{RU_DICTIONARY.productPage.unitOfMeasure}</span>
                                                    <span className="text-sm text-gray-900">{product.unit_of_measure}</span>
                                                </div>
                                            )}
                                            {product.specifications?.material && (
                                                <div className="flex py-3">
                                                    <span className="text-sm text-gray-500 w-40 flex-shrink-0 font-medium">{RU_DICTIONARY.productPage.material}</span>
                                                    <span className="text-sm text-gray-900">{product.specifications.material}</span>
                                                </div>
                                            )}
                                            {product.specifications?.weight_kg && (
                                                <div className="flex py-3">
                                                    <span className="text-sm text-gray-500 w-40 flex-shrink-0 font-medium">{RU_DICTIONARY.productPage.weight}</span>
                                            ...
                                            {product.specifications?.color && (
                                                <div className="flex py-3">
                                                    <span className="text-sm text-gray-500 w-40 flex-shrink-0 font-medium">{RU_DICTIONARY.productPage.color}</span>
                                                    <span className="text-sm text-gray-900 capitalize">{product.specifications.color}</span>
                                                </div>
                                            )}
                                            {product.specifications?.grade && (
                                                <div className="flex py-3">
                                                    <span className="text-sm text-gray-500 w-40 flex-shrink-0 font-medium">{RU_DICTIONARY.productPage.grade}</span>
                                                    <span className="text-sm text-gray-900">{product.specifications.grade}</span>
                                                </div>
                                            )}
                                            {product.specifications?.shelf_life_months && (
                                                <div className="flex py-3">
                                                    <span className="text-sm text-gray-500 w-40 flex-shrink-0 font-medium">{RU_DICTIONARY.productPage.shelfLife}</span>
                                                    <span className="text-sm text-gray-900">{product.specifications.shelf_life_months} {RU_DICTIONARY.productPage.months}</span>
                                                </div>
                                            )}
                                            {(product.specifications?.length_cm || product.specifications?.width_cm || product.specifications?.height_cm) && (
                                                <div className="flex py-3">
                                                    <span className="text-sm text-gray-500 w-40 flex-shrink-0 font-medium">{RU_DICTIONARY.productPage.dimensions}</span>
                                            ...
                                            {product.sku && (
                                                <div className="flex py-3">
                                                    <span className="text-sm text-gray-500 w-40 flex-shrink-0 font-medium">{RU_DICTIONARY.productPage.sku}</span>
                ...
                <div className="relative isolate w-full mt-6 pt-6 pb-6 overflow-hidden">
                    ...
                    <div className="mx-auto max-w-[1440px] px-4 space-y-6">
                        ...
                        <LazySection
                            minHeight="400px"
                            rootMargin="400px"
                            skeleton={<SkeletonProductRow title={RU_DICTIONARY.productPage.similarProducts} />}
                        >
                        ...
                        <LazySection
                            minHeight="400px"
                            rootMargin="400px"
                            skeleton={<SkeletonProductRow title={RU_DICTIONARY.productPage.bestseller} />}
                        >
                            <LazyBestSellers
                                title={RU_DICTIONARY.productPage.bestseller}
